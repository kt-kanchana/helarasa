      ...
      <head>
        ...
        <style>
          ...
          table {
            ...
            font-size: 9px;
            margin: 4px 0;
            color: #444;  /* Slightly darker gray for table text */
          }
          th, td {
            padding: 2px;
            text-align: left;
          ...
          hr {
            border: none;
            border-top: 1px dashed #2196F3;  /* Blue dashed lines */
            margin: 4px 0;
          }
          .total {
            font-size: 12px;
            text-align: right;
            margin: 3px 0;
            color: #1976D2;  /* Darker blue for total */
            font-weight: bold;
          }
          .thanks {
            font-size: 10px;
            text-align: center;
            margin-top: 5px;
            color: #2196F3;  /* Blue thank you message */
          }
          .cut {
            font-size: 8px;
            text-align: center;
            margin-top: 10px;
            color: #666;  /* Gray for cut line */
          }
        </style>
        <script>
...
function populateCategories() {
  ...
  categoryList.innerHTML = '<h3>Existing Categories:</h3><ul></ul>';
...
function populateItems() {
  ...
  itemList.innerHTML = '';
  updateItem.innerHTML = '<option value="">Select Item</option>';

  if (itemCategory && menu[itemCategory]) {
    itemList.innerHTML = '<h3>Items in ' + itemCategory + ':</h3><table><tr><th>Name</th><th>Price</th><th>Action</th></tr></table>';
    const table = itemList.querySelector('table');
    menu[itemCategory].forEach((item, index) => {
      const row = table.insertRow();
      row.insertCell(0).textContent = item.name;
      row.insertCell(1).textContent = `LKR ${item.price}`;
      const actionCell = row.insertCell(2);
      const removeBtn = document.createElement('button');
      removeBtn.textContent = 'Remove';
      removeBtn.onclick = () => removeItemFromCategory(itemCategory, index);
      actionCell.appendChild(removeBtn);

      const option = document.createElement('option');
      option.value = JSON.stringify({ category: itemCategory, index });
      option.textContent = `${item.name} - LKR ${item.price}`;
      updateItem.appendChild(option);
    });
  }
}
...
function removeItemFromCategory(category, index) {
  if (confirm(`Are you sure you want to remove ${menu[category][index].name}?`)) {
...
function updatePrice() {
  const selectedItemJson = document.getElementById('updateItem').value;
  const newPrice = parseInt(document.getElementById('newPrice').value);

  if (selectedItemJson && !isNaN(newPrice) && newPrice >= 0) {
    const { category, index } = JSON.parse(selectedItemJson);
    menu[category][index].price = newPrice;
...
if (window.location.pathname.includes('admin.html')) {
  document.getElementById('itemCategory').addEventListener('change', populateItems);
  populateCategories();
  populateItems();
}
